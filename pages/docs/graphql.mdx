...
This is a function that returns a custom React hook and has full TypeScript support
...
<Callout>
  The code above will show the following error: 
  
  ```txt
  Argument of type '"$user"' is not assignable to parameter of type '"tasks" | "user"'
  ```
</Callout>
